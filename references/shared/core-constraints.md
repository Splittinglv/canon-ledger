...
## 爽点与节奏（可选，按题材 profile 调整）

- 不强制每章都有打脸/装逼爽点。章纲「爽点」可写「无」，不算失败。
- 「整章无收获」指没有目标、代价或变化，不等于「必须有爽点」。过渡章可以只推进信息、关系或时间。
- 若本卷明确走爽文，组合爽点与里程碑可用滚动窗口（5章/10-15章）预警，不作逐章硬判。
- 连续同类型兑现达到 3 章时记为风险预警，优先换类型或换执行方式。
