...
> **当前分支 `plot-facts-only`**：写章只守事实和剧情。不跑网文润色、Anti-AI 终检、风格适配。文风只看书项目里的 `设定集/文风提示词.md`，作者手改即可。规划不强制每章爽点；章纲可写「无」。
...
打开 `设定集/文风提示词.md`，在「作者提示词」下写下你的要求（视角、句长、禁忌修辞、想贴近的作品）。留空则按当前模型默认写，插件不会再套网文腔。
...
| plot-facts-only 分支 | 写章去掉润色/Anti-AI/风格适配；文风只读 `设定集/文风提示词.md`。 |
