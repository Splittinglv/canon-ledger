...
5. retired state/index（兼容诊断，不是当前事实）。

`.story-system` 大纲和章合同提供未来剧情方向，不能证明事件已经发生。磁盘上尚未认证的长期设定属于 draft setting，不能覆盖 active author axioms。
...
公开分层入口固定为：active 使用 `canon-v3 history` 或 `/api/canon-v3/*` 事实接口；STAGING 只看 `canon-v3 status` 的 `cases[].review_material`；style 只看 `style-memory show`。入口没有暴露的数据 fail-closed，不绕过它直接读取对象库。
