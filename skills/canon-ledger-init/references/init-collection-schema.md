...
> `/canon-ledger-init` 在 Deep 采集阶段按本结构逐项收集信息；字段齐全并过「充分性闸门」后再执行 `canon_ledger.py init`。本文件是字段参考，不是执行脚本。
...
| 字段组 | 对应 Step | 说明 |
|--------|-----------|------|
| `project.*` | Step 2 | 故事核（书名/题材/规模/一句话/核心冲突；读者/平台不说就不问） |
| `protagonist.*` | Step 3 | 主角骨架（姓名/欲望/缺陷/原型/结构） |
| `relationship.*` | Step 3 | 感情线与对立关系事实（不说就不问） |
