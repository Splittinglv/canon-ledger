...
| `genesis_import.world.*` | 同名 world/currency/sect/cultivation 参数 | 非空初始硬事实进入 genesis admission |
...
3. 统一 init 工具将净化后的 `MASTER_SETTING.initial_canon` 经 verified native snapshot 导入 genesis。这些事实在 `canon-v3 author-axioms` 中显示为 `genesis_admissions`，不是 managed author-axiom `records`。
