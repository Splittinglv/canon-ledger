---
name: webnovel-init
description: 深度初始化网文项目：分阶段收集书名、题材、主角与世界观，生成设定集、总纲和 Story System。用户说初始化网文、开新书、/webnovel-init 时使用。
---
...
- 禁止在插件目录（`${WEBNOVEL_PLUGIN_ROOT}`）下生成项目文件；禁止直接把 `WORKSPACE_ROOT` 当作 `PROJECT_ROOT`，除非用户明确指定当前目录就是书项目根。
