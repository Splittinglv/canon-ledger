...
写前 context-agent 按共享协议完整读取同一版本的上下文分页，再整理本章相关约束。
使用 `--all-pages` 一次导出，不对每一页重复加载 runtime。
交付预算超限不等于事实不完整；保持原 Canon gate，先分页或调整容量，不因默认 4,000
预算直接停止写作。省略上章已经明确的境界等前态也不要求为 schema 重写正文。
