...
def _resolve_project_root_from_pointer(cwd: Path, *, stop_at: Optional[Path] = None) -> Optional[Path]:
    ...
    - relative path is also supported (resolved relative to the pointer file's parent dir).
    """
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    if ws_root is None:
        env_ws = _workspace_dir_env()
        if env_ws:
            ws_root = normalize_windows_path(env_ws).expanduser()
            try:
    ...
    if ws_root is not None:
        ...
        if explicit_workspace or has_cursor_dir or has_claude_dir:
            try:
                ...
                cursor_pointer = ws_root / CURSOR_PROJECT_POINTER_REL
                cursor_pointer.write_text(str(root), encoding="utf-8")
