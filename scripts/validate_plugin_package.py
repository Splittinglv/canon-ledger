...
def _check_manifest(root: Path, issues: list[dict[str, str]]) -> tuple[str, str]:
    ...
    if error:
        issues.append(_issue("manifest.plugin_json", message=error, path=str(plugin_json), repair="恢复 .cursor-plugin/plugin.json 或 .claude-plugin/plugin.json。"))
        return "", ""
...
def validate_package(root: str | Path | None = None, *, strict: bool = False) -> dict[str, Any]:
    if root is not None:
        ...
        repo_root = plugin_root if (plugin_root / "scripts" / "webnovel.py").is_file() else here.parent.parent.parent
