...
class HumanReviewService:
        ...
        rendered = [
            {
                **item,
                "status": _text(
                    (decisions.get(item["decision_id"]) or {}).get("action"),
                    40,
                )
                or "pending",
            }
    ...
    def apply_decisions(
        ...
        # model-supplied ``verified`` flag; only a chapter-bound decision below
        # may promote an event from supported to verified.
        events = [
            {**dict(event), "verification": "supported"}
        ...
        return {
            ...
            "verified_event_ids": [item for item in verified_event_ids if item],
    ...
    def record(self, payload: Any) -> dict[str, Any]:
        ...
        for raw in raw_decisions:
            ...
            if action == "replace" and not isinstance(replacement, dict):
    ...
    def list_items(self, chapter: int | None = None) -> list[dict[str, Any]]:
        ...
        for path in paths:
            ...
            for raw in payload.get("items") or []:
                ...
                item["status"] = _text((decision or {}).get("action"), 40) or "pending"
                result.append(item)
        return result
