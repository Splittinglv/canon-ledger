...
# -*- coding: utf-8 -*-
"""
CanonLedger 统一入口（面向 skills / agents 的稳定 CLI）

设计目标：
- 只有一个入口命令，避免到处拼 `python -m data_modules.xxx ...` 导致参数位置/引号/路径炸裂。
- 自动解析正确的 book project_root（包含 `.canon-ledger/state.json` 的目录）。
...
def _scripts_dir() -> Path:
    # data_modules/canon_ledger.py -> data_modules -> scripts
...
def _project_root_diagnostic(
    ...
    if explicit_project_root:
        return (
            "未找到有效书项目根目录（需要包含 .canon-ledger/state.json）: "
            f"{explicit_project_root}\n"
            f"detail: {exc}"
        )
    return (
        "当前工作区还没有激活的书项目（未找到 .canon-ledger/state.json）。\n"
        "请先运行 canon_ledger.py init 创建项目，或运行 canon_ledger.py use <project_root> 绑定已有书项目。\n"
        f"detail: {exc}"
    )
