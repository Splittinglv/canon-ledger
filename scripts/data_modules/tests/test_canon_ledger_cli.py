...
def test_canon_ledger_commit_forwards(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(module, "_run_script", _fake_run_script)
    monkeypatch.setattr(sys, "argv", ["canon-ledger", "--project-root", str(project_root), "chapter-commit", "--chapter", "3"])
...
def test_canon_ledger_story_events_forwards(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["canon-ledger", "--project-root", str(project_root), "story-events", "--chapter", "3"],
    )
...
def test_preflight_succeeds_for_valid_project_root(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["canon-ledger", "--project-root", str(project_root), "preflight"])
...
def test_preflight_includes_story_runtime_health(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["canon-ledger", "--project-root", str(project_root), "preflight", "--format", "json"],
    )
...
def test_project_status_cli_outputs_json_without_reusing_status(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["canon-ledger", "--project-root", str(project_root), "project-status", "--format", "json"],
    )
    ...
    assert int(exc.value.code or 0) == 0
    assert report["schema_version"] == "canon-ledger-project-status/v1"
...
def test_run_ledger_cli_records_and_reports_resume(monkeypatch, tmp_path, capsys):
    ...
    assert int(exc.value.code or 0) == 0
    assert resume["schema_version"] == "canon-ledger-run-ledger/v1"
...
def test_doctor_cli_reports_missing_init_file(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["canon-ledger", "--project-root", str(project_root), "doctor", "--format", "json"],
    )
    ...
    report = json.loads(captured.out)
    assert int(exc.value.code or 0) == 1
    assert report["schema_version"] == "canon-ledger-doctor/v1"
...
def test_status_command_still_forwards_to_status_reporter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(module, "_run_script", _fake_run_script)
    monkeypatch.setattr(sys, "argv", ["canon-ledger", "--project-root", str(project_root), "status", "--focus", "all"])
...
def test_write_gate_cli_runs_prewrite(monkeypatch, tmp_path, capsys):
    ...
    report = json.loads(captured.out)
    assert int(exc.value.code or 0) == 0
    assert report["schema_version"] == "canon-ledger-write-gate/v1"
...
def test_projections_retry_cli_runs(monkeypatch, tmp_path, capsys):
    ...
    write_current_chapter_contract(project_root, 1)
    ...
    report = json.loads(captured.out)
    assert int(exc.value.code or 0) == 0
    assert report["schema_version"] == "canon-ledger-projections/v1"
...
def test_preflight_reports_empty_workspace_without_traceback(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.delenv("CANON_LEDGER_PROJECT_ROOT", raising=False)
    monkeypatch.delenv("CURSOR_PROJECT_DIR", raising=False)
    monkeypatch.setattr(sys, "argv", ["canon-ledger", "preflight", "--format", "json"])
...
def test_quality_trend_report_writes_to_book_root_when_input_is_workspace_root(tmp_path, monkeypatch):
    ...
    (workspace_root / ".cursor" / "canon-ledger-current-project").write_text(
        str(book_root), encoding="utf-8"
    )
    ...
    assert (book_root / ".canon-ledger" / "index.db").is_file()
    assert not (workspace_root / ".canon-ledger" / "index.db").exists()
...
def test_review_pipeline_builds_artifacts(tmp_path):
    ...
    chapter_binding_path = project_root / ".canon-ledger" / "tmp" / "chapter_binding.json"
...
def test_review_pipeline_minimal_cli_writes_explicit_skipped_artifact(
    ...
    review_results_path = project_root / ".canon-ledger" / "tmp" / "review_results.json"
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_review_pipeline_rejects_changed_manuscript_before_side_effects(tmp_path):
    ...
    binding_path = project_root / ".canon-ledger" / "tmp" / "chapter_binding.json"
    binding_path.parent.mkdir(parents=True, exist_ok=True)
    binding_path.write_text(json.dumps(binding, ensure_ascii=False), encoding="utf-8")
    review_path = project_root / ".canon-ledger" / "tmp" / "review_results.json"
    original_review = {
        "chapter": 20,
        "chapter_binding": binding,
        "issues": [
...
def test_review_pipeline_main_creates_output_directories(tmp_path):
    ...
    audit_out = project_root / ".canon-ledger" / "tmp" / "review" / "audit.json"
    ...
    with sqlite3.connect(project_root / ".canon-ledger" / "index.db") as conn:
        row = conn.execute(
...
def test_canon_ledger_skill_flow_runs_story_contract_context_and_review_pipeline_with_stubbed_vector_model(
    ...
    refs_dir = project_root / ".cursor" / "references"
    ...
    write_current_chapter_contract(project_root, 2)
    ...
    def _run_canon_ledger(argv):
        monkeypatch.setattr(sys, "argv", ["canon-ledger", *argv])
        with pytest.raises(SystemExit) as exc:
            module.main()
    ...
    audit_out = project_root / ".canon-ledger" / "tmp" / "review_audit.json"
    assert (
        _run_canon_ledger(
            [
                "--project-root",
...
def test_subagent_models_cli_reads_project_file(monkeypatch, tmp_path, capsys):
    ...
    (book_root / ".canon-ledger" / "subagent-models.json").write_text(
...
def test_subagent_models_cli_without_book_still_succeeds(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["canon-ledger", "subagent-models", "--format", "json"])
