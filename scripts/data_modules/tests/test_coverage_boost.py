...
def test_compactor_keeps_old_timeline_events_exactly():
    """时间线即使超过软容量，也不得被摘要替换或删除。"""
    data = ScratchpadData.empty()
    for chapter in range(1, 62):
        data.timeline.append(
            _make_item(
                f"时间线-{chapter}",
                category="timeline",
                subject=f"第{chapter}章事件",
                field="event",
                value=f"第{chapter}章的精确事实",
                chapter=chapter,
            )
        )
    ...
    assert [row.id for row in result.timeline] == [
        f"时间线-{chapter}" for chapter in range(1, 62)
    ]
    assert result.timeline[0].value == "第1章的精确事实"
    assert result.timeline[-1].value == "第61章的精确事实"
