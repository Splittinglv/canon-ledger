...
def test_project_status_json_shape(tmp_path):
    ...
    assert report["phase"] == "canon_v3:initialization_required"
    assert report["target_chapter"] == 1
    assert report["blocking"] == ["initialization_required"]
...
def test_project_status_summary_is_short_and_machine_source_is_json(tmp_path):
    ...
    assert "阶段：canon_v3:initialization_required" in summary
